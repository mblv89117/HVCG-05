...
$seed = @(
  @{ Title = 'ACCG Inc.'; ClientCode = 'ACCG01'; DBA = 'ACCG'; Industry = 'Services'; ClientStage = 'Active Client'; IsActive = $true; Notes = 'Imported from Atlas Client 360 / Microsoft' },
  @{ Title = 'Prodigy Games LLC'; ClientCode = 'PDG01'; DBA = 'Prodigy Games'; Industry = 'Gaming'; ClientStage = 'Active Client'; IsActive = $true; Notes = 'Imported from Atlas Client 360 / Microsoft' },
  @{ Title = 'Colorado Craft Beef'; ClientCode = 'CCB01'; DBA = 'Colorado Craft Beef'; Industry = 'Food & Beverage'; ClientStage = 'Active Client'; IsActive = $true; Notes = 'Imported from Atlas Client 360 / Microsoft' },
  @{ Title = 'That''s Kava LLC'; ClientCode = 'KAVA01'; DBA = 'That''s Kava'; Industry = 'Food & Beverage'; ClientStage = 'Former Client'; IsActive = $false; Notes = 'Queued for import — confirm source match' },
  @{ Title = 'Christie''s Place LLC'; ClientCode = 'CPL01'; DBA = 'Christie''s Place'; Industry = 'Nonprofit'; ClientStage = 'Former Client'; IsActive = $false; Notes = 'Queued for import — confirm source match' },
  @{ Title = 'Hart Family Dental'; ClientCode = 'HFD01'; DBA = 'Hart Family Dental'; Industry = 'Healthcare'; ClientStage = 'Active Client'; IsActive = $true; Notes = 'Queued for import — confirm source match' },
  @{ Title = 'Lien Partners'; ClientCode = 'LIEN01'; DBA = 'Lienpartners'; Industry = 'Professional Services'; ClientStage = 'Active Client'; IsActive = $true; Notes = 'Imported from Atlas Client 360 / Microsoft' }
)
...
if ($Client360JsonPath -and (Test-Path $Client360JsonPath)) {
  ...
  foreach ($c in @($c360.candidates + $c360.clients)) {
    ...
    $seed += @{
      Title       = $title
      ClientCode  = $code
      ClientStage = if ($c.lifecycle -eq 'active') { 'Active Client' } elseif ($c.lifecycle -eq 'former') { 'Former Client' } else { 'Prospect' }
      IsActive    = ($c.lifecycle -eq 'active')
      Notes       = "Imported from Client 360 id=$($c.id)"
    }
  }
}
...
foreach ($row in $seed) {
  ...
  if ($row.Notes) { $values.Notes = [string]$row.Notes }
