...
describe('Ask Atlas drawer prompt summary', () => {
  ...
  it('does not promote LIKELY items to CONFIRMED in drawer copy', () => {
    const answer = summarizeAskAtlasPrompt('What is overdue?', items);
    assert.match(answer, /LIKELY/);
    assert.equal(answer.includes('(CONFIRMED)'), false);
  });
});
