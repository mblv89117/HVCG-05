...
import type { ChartDatum, Metric, Notification, Tone } from '../types'
...
export function NotificationList({ notifications, onRead, limit }: { notifications: Notification[]; onRead: (id: string) => void; limit?: number }) {
  return (
    <div className="notification-list">
      {notifications.slice(0, limit).map((item) => (
        <button key={item.id} className={`notification-item ${item.read ? 'is-read' : ''}`} onClick={() => onRead(item.id)}>
          <span className={`notification-dot severity-${item.severity.toLowerCase()}`} />
          <span className="notification-copy">
            <span className="notification-meta"><Badge tone={item.severity === 'Critical' ? 'critical' : item.severity === 'Action' ? 'warning' : 'neutral'}>{item.domain}</Badge><small>{item.timestamp}</small></span>
