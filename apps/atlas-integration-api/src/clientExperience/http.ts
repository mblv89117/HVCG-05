/**
 * Client experience HTTP — isolated /api/client/* plus operator staging.
 */
...
export async function handleClientExperience(opts: {
  ...
  try {
    if (stagedCode) {
      if (opts.method === 'GET') {
        send(opts.res, 200, operatorExperienceStatus({
      ...
      send(
        ...
        result.replay ? 200 : 201,
        {
          ...
          note: 'Invitation is record-only. LIVE_GTM_OUTBOUND stays OFF. Token is shown once and is not emailed.',
        },
        opts.origin,
      );
      return true;
    ...
    if (opts.method === 'GET' && opts.path === '/api/client/requests') {
      const view = buildClientWorkspaceView({ dataDir: opts.cfg.dataDir, principal });
      send(opts.res, 200, { requests: view.requests, attention: view.attention, clientCode: view.clientCode }, opts.origin);
      return true;
    }
