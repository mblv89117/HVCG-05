...
export function bootstrapKnownProjects(
  ...
  for (const kp of KNOWN_PROJECTS) {
    ...
    const project: ProjectRecord = {
      ...
      // Do not invent a due date for bootstrapped initiatives.
    ...
    // Owner must establish real milestones — do not seed fabricated "Scope confirmed".
    // Seed a single explicit "needs definition" task so My Work surfaces the gap.
    const task: TaskRecord = {
      id: crypto.randomUUID(),
      title: `Next action required — ${kp.name}`,
      description: `Confirm current phase, blockers, and the single next action for ${kp.name}. Do not treat this bootstrap task as assessed project health.`,
      projectId: id,
      clientId,
      clientName: kp.clientName,
      assigneeKind: 'person',
      ...
      checklist: [],
      nextAction: 'Open project and set a real next action',
      confidence: 0.5,
      autoGenerated: true,
      businessEntity: kp.businessEntity,
      createdAt: now,
