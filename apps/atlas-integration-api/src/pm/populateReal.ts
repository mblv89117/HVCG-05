...
function findOrCreateClientProject(
  ...
  const project: ProjectRecord = {
    ...
    targetCompletionDate: addDays(45),
    currentPhase: c.lifecycle === 'active' ? 'Delivery' : 'Pipeline',
    status: 'active',
    priority: inferPriority(c),
    health: (c.completenessScore || 0) < 40 ? 'watch' : 'healthy',
    progressPercent: Math.min(85, Math.max(5, c.completenessScore || 10)),
    nextAction: c.recommendedNextActions?.[0] || `Review ${c.displayName} and set next milestone`,
...
export function populateRealWorkFromMicrosoft(
  ...
  for (const p of pm.listProjects().filter((x) => x.status === 'active')) {
    ...
    else if (open.length === 0) health = 'watch';
    else health = 'healthy';

    const next =
      open.sort((a, b) => (a.dueDate || '9999').localeCompare(b.dueDate || '9999'))[0]?.title ||
      p.nextAction;
