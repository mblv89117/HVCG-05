...
import { createAuthorizedCapitalStore, createSharePointCapitalService } from './capital/backend.ts';
import { createAuthorizedPmRepository, createSharePointPmService } from './pm/backend.ts';
import { startConfiguredFabricSweep } from './pm/sharepoint/fabric/sweep.ts';
  ...
  server.listen(cfg.port, cfg.host, () => {
    console.info(
      JSON.stringify({
        ...
        capitalBackend: cfg.capitalBackend.mode,
        capitalClassification: cfg.capitalBackend.classification,
        dataDir: cfg.dataDir,
