...
import { isClientOnlyPrincipal } from '../src/clientExperience/roles.ts';
  ...
  it('stages, invites, authenticates, exchanges documents, and fail-closes cross-client access', async () => {
    await withHub(async ({ base }) => {
      ...
      const stagedABody = (await stagedA.json()) as {
        workspace: { clientCode: string; entraGroupProvisioned: boolean; outboundInviteSent: boolean; gccWorkspaceKey: string };
      ...
      const requestsBody = (await requests.json()) as { attention: Array<{ id: string; kind: string }>; clientCode: string };
