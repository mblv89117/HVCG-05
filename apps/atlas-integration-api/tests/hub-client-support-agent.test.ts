 ...
 * + ATLAS-CLIENT-SUPPORT-RELATED-CAPITAL-001 entitled same-scope
 * capital-prepare refs (same RelatedDocumentCapitalRef / relatedCapital
 * path as research-intel / onboarding relatedCapital). Inverse of
...
function assertSupportRelatedCapitalHonesty(item: ClientSupportAgentRecord): void {
  assert.equal(item.invented, false);
  assert.equal(item.hubMiRow, false);
  assert.equal(item.execute, false);
  assert.equal(item.send, false);
...
describe('ATLAS-CLIENT-SUPPORT-RELATED-CAPITAL-001 entitled same-scope inverse', () => {
  it('attaches same-scope relatedCapital on entitled client support and get_client_context', async () => {
    const result = await searchAuthorizedKnowledge({
      principal: staff,
      picture: emptyHonestOperatingPicture(),
      searchQuery: 'SYN01',
    ...
    const mail = result.authorizedSearch.clientSupport.items.find((row) => row.id === 'mail-syn-1');
    assert.ok(mail);
    const capital = mail.relatedCapital?.find((row) => row.id === 'cap-syn-1');
    assert.ok(capital);
    ...
    assert.equal('downloadUrl' in capital, false);
    assert.equal('transcript' in capital, false);
    assert.ok((mail.relatedCapital?.length || 0) <= DOCUMENT_RELATED_CONTEXT_PAGE_SIZE);
    assert.equal(mail.relatedMeetings?.some((row) => row.id === 'meet-syn-1'), true);
    assert.equal(mail.relatedDocuments?.some((row) => row.id === 'file-syn-1'), true);
    assert.equal(mail.relatedProjects?.some((row) => row.id === 'proj-syn-1'), true);
    assert.equal(mail.relatedThreads?.some((row) => row.id === 'mail-follow-1'), true);
    ...
    assert.equal(
      /downloadUrl|transcript|attendee|TargetAmount|suggestedDraft/i.test(JSON.stringify(mail.relatedCapital)),
      false,
    );
    assert.equal(JSON.stringify(mail.relatedCapital).includes('PDG01'), false);
    ...
    const ctxMail = viaIndex.clientContext.clientSupport.items.find((row) => row.id === 'mail-syn-1');
    assert.ok(ctxMail);
    assert.equal(ctxMail.relatedCapital?.some((row) => row.id === 'cap-syn-1'), true);
    assert.equal(ctxMail.relatedThreads?.some((row) => row.id === 'mail-follow-1'), true);
    assert.equal(ctxMail.relatedDocuments?.some((row) => row.id === 'file-syn-1'), true);
    assert.equal(ctxMail.relatedProjects?.some((row) => row.id === 'proj-syn-1'), true);
    assert.equal(ctxMail.relatedMeetings?.some((row) => row.id === 'meet-syn-1'), true);
  ...
  it('honestly omits relatedCapital when none are entitled', async () => {
    ...
    const mail = result.authorizedSearch.clientSupport.items.find((row) => row.id === 'mail-syn-1');
    assert.ok(mail);
    assert.equal(mail.relatedCapital, undefined);
    assert.equal('relatedCapital' in mail, false);
    assert.equal(mail.relatedMeetings?.some((row) => row.id === 'meet-syn-1'), true);
    assert.equal(mail.relatedDocuments?.some((row) => row.id === 'file-syn-1'), true);
    assert.equal(mail.relatedProjects?.some((row) => row.id === 'proj-syn-1'), true);
    assert.equal(mail.relatedThreads?.some((row) => row.id === 'mail-follow-1'), true);
  ...
  it('never attaches Client B capital to a Client A support item', async () => {
    const result = await searchAuthorizedKnowledge({
      principal: staff,
      picture: emptyHonestOperatingPicture(),
      searchQuery: 'SYN01',
    ...
    assert.equal((mail.relatedCapital || []).some((row) => /pdg/i.test(row.id) || /pdg/i.test(row.title)), false);
    const blob = JSON.stringify(result.authorizedSearch.clientSupport);
    assert.equal(blob.includes('PDG01'), false);
    assert.equal(blob.includes('ACCG01'), false);
    assert.equal(blob.includes('CCB01'), false);
  ...
  it('unscoped lender catalog titles never attach scoped capital', async () => {
    ...
    assert.equal(result.authorizedSearch.clientSupport.items.length, 0);
    assert.equal(
      result.authorizedSearch.clientSupport.items.some((row) => row.relatedCapital),
      false,
    );
    const catalogBlob = JSON.stringify(result.authorizedSearch.clientSupport);
  ...
  it('omits extras for unauthorized or other-client principals', async () => {
    const unknown = await searchAuthorizedKnowledge({
      ...
      entitledSearch: async (query) => ({
        query,
        results: [syn01ClientHit(), syn01SupportHit(), syn01CapitalHit()],
      }),
    });
    assert.equal(unknown.authorizedSearch.entitled, false);
    assert.equal(unknown.authorizedSearch.clientSupport.items.length, 0);
    assert.equal(
      unknown.authorizedSearch.clientSupport.items.some((row) => row.relatedCapital),
      false,
    );
    const unknownBlob = JSON.stringify(unknown.authorizedSearch.clientSupport);
  ...
  it('leaves the empty client-support payload unchanged and omits relatedCapital when search.capitalSubmissions is absent', () => {
    const empty = emptyClientSupportPayload();
    assert.deepEqual(empty.items, []);
    ...
    assert.equal(absent.relatedThreads?.some((row) => row.id === 'mail-follow-1'), true);
    assert.equal(absent.relatedDocuments?.some((row) => row.id === 'file-syn-1'), true);
    assert.equal(absent.relatedProjects?.some((row) => row.id === 'proj-syn-1'), true);
    assert.equal(
      absent.researchRelationship?.some((row) => row.clientCode === 'SYN01'),
  ...
  it('never invents TargetAmount, lender criteria, fit, financing status, downloadUrl, or Hub-MI', async () => {
    ...
    assert.equal(/TargetAmount/i.test(blob), false);
    assert.equal(/Hub-MI/i.test(blob), false);
    assert.ok(mail.relatedCapital?.some((row) => row.id === 'cap-syn-1'));
    const attached = mail.relatedCapital?.find((row) => row.id === 'cap-syn-1');
    assert.ok(attached);
    ...
    assert.equal(JSON.stringify(mail.relatedCapital).includes('5000000'), false);
    assert.equal(mail.invented, false);
    assert.equal(mail.hubMiRow, false);
    assert.equal(mail.execute, false);
    assert.equal(mail.send, false);
  ...
  it('still attaches relatedMeetings, researchRelationship, relatedDocuments, relatedProjects, and relatedThreads next to relatedCapital', async () => {
    ...
    assert.equal(JSON.stringify(mail.relatedMeetings).includes('PDG01'), false);
    assert.equal(JSON.stringify(mail.researchRelationship).includes('PDG01'), false);
    assert.equal(/TargetAmount/i.test(JSON.stringify(mail.relatedCapital)), false);
    assert.equal(/downloadUrl|transcript|suggestedDraft/i.test(JSON.stringify(mail.relatedCapital)), false);
    assert.equal(mail.invented, false);
    assert.equal(mail.hubMiRow, false);
    assert.equal(mail.execute, false);
    assert.equal(mail.send, false);
