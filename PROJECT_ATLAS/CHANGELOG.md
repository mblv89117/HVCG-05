...
| Date | Change |
|------|--------|
| 2026-08-19 | **Hub Phase 5E persist-consistency.** LIVE Hub `9091dd5` / deploy `3610366d`. SYN Graph 503 no longer mutates operator-visible state without a durable submission row. Recorded submit returns 200 overlay-authoritative. Elite stays `2a4e115`. Convert `887edd8` remains repo-only. |
| 2026-08-19 | **Hub Phase 5D confirmation-gate P1 CLOSED.** Then-LIVE Hub `b333fb4` / deploy `5d3826c0` (superseded same day by `9091dd5`). Mismatched lenderId recorded-submit 422. Elite stays `2a4e115`. |
| 2026-08-19 | **Elite Phase 5B live.** SWA `2a4e115` / `index-CiVmQVqq.js` = Capital `b9806bc` + Client `0ffb645`. Then-LIVE Hub `a43803e` / deploy `3d406e37` (superseded same day by `b333fb4`). CRM operator LIVE; Owner-browser Premium UI certified Phase 5C. stash0 `773e120` **not applied**. Search P2 SYN **15–24s**. `origin/main` stays `b641fdd`. ACCG01 ACL Apply was not run. |
| 2026-08-18 | **Documentation honesty (LIVE vs CANDIDATE).** Then-LIVE Elite `e5740379` / `index-DvEHjcS6.js`. Then-LIVE Hub `d22b55f` / Azure deploy `501fb29b-80f6-427d-8c65-3f1a88da52d9`. CRM operator `a43803e` was still a candidate. Superseded 2026-08-19 by the CRM exact-SHA deploy. `origin/main` stays `b641fdd`. ACCG01 ACL Apply was not run. Docs-only; no deploy. |
