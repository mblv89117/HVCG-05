...
Orchestration `agentId`: **`deployment-manager`**

1. Confirm worktree/branch.
2. `bash scripts/orchestration/list-ready.sh --agent deployment-manager`
3. Claim Ready work before editing deployment/infrastructure paths; otherwise heartbeat `Idle`.
4. Production remains owner-gated; Track 1 freeze facts are Deployment-owned.
...
3. Run orchestration `list-ready` for `deployment-manager` before inventing work.
4. Read `.worktrees/deployment-engineer/docs/deployment/DEPLOYMENT_ENGINEER_HANDOFF.md` + Track-1 README + GO_LIVE_STATUS (same worktree).
5. Verify PAC profile `HVCG-Dev-Maker` before any command.
