...
Orchestration `agentId`: **`master-pm`**

1. Confirm worktree/branch.
2. `bash scripts/orchestration/list-ready.sh --agent master-pm`
3. Claim Ready work before editing; otherwise heartbeat `Idle`.
4. Prefer `PROJECT_ATLAS/ORCHESTRATION/` as work SoR; `.agent-comms/` remains the message bus.
...
3. Run orchestration `list-ready` for `master-pm` before inventing work.
4. Read [AGENT_HANDOFF.md](../AGENT_HANDOFF.md) → OWNER_DECISIONS (master-pm worktree) → go-live + funnel status.
5. Run `./scripts/agent-comms/master-dashboard.sh` if the bus is live.
