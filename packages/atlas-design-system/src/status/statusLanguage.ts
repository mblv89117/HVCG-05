/**
 * Atlas global status language (design-system source).
 * Labels must stay identical to Elite `apps/atlas-elite-os/src/ui/statusLanguage.ts`.
 * Hub/backend remains authoritative for classification; this module is display only.
 */
...
export type AtlasStatusKey = keyof typeof ATLAS_STATUS;
export type AtlasStatusLabel = (typeof ATLAS_STATUS)[AtlasStatusKey];
...
export function atlasStatusChip(label: string): { label: string; tone: AtlasStatusTone } {
