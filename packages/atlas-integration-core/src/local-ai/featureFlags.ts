 ...
 * - default / unset: Local AI disabled (Atlas must boot without Ollama, models, or .data)
 * - development: still Off unless LOCAL_AI_ENABLED=true is set explicitly
 * - staging: Off unless explicitly enabled for a controlled local-provider test
 * - production: must remain Off; do not make Atlas depend on a developer workstation
 * - unavailable provider: flags may be On, but the Hub adapter reports available=false
 *   and fails soft (no fake completions)
 *
