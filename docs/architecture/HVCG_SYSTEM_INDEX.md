...
- **Capital Operations:** Internal Atlas module (Elite `/capital`, Hub, `HVCG_*` capital lists). **Not an eighth system.** Min-slice lists exist. **LIVE Hub** `/health` reports `capitalBackend.mode=sharepoint` with durable overlay (`a43803e`, deploy `3d406e37-2d91-4fd6-a20b-8c955c7b5733`). Elite post-shortlist UI is in SWA `2a4e115` (includes `b9806bc`). **ACCG01 ACL Apply was not run.** Schema/docs: `docs/CAPITAL_OPERATIONS_DISCOVERY.md`; live SHAs: `docs/CAPITAL_RELEASE_PROVENANCE.md`.
- **Production URL/status:** LIVE Elite `2a4e115` (`index-CiVmQVqq.js` = Capital `b9806bc` + Client `0ffb645`) at `https://zealous-rock-0090c7e1e.7.azurestaticapps.net`; LIVE Hub `a43803e` at `https://app-atlas-integration-hub.azurewebsites.net` (deploy `3d406e37`). Do not treat `origin/main` (`b641fdd`) as Azure production. CRM operator is **LIVE DEPLOYED**; signed-in Premium UI **HOLD**. stash0 `773e120` is **not applied**. Command-K SYN search P2 (**15–24s**) is not fixed.
