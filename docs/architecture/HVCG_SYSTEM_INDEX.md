...
- **Capital Operations:** Internal Atlas module (Elite `/capital`, Hub, `HVCG_*` capital lists). **Not an eighth system.** Min-slice lists exist. **LIVE Hub** `/health` reports `capitalBackend.mode=sharepoint` with durable overlay (`b333fb4`, deploy `5d3826c0-f784-41da-9043-6912e63e122e`). Confirmation-gate P1 **CLOSED**. Elite post-shortlist UI is in SWA `2a4e115` (includes `b9806bc`). **ACCG01 ACL Apply was not run.** Schema/docs: `docs/CAPITAL_OPERATIONS_DISCOVERY.md`; live SHAs: `docs/CAPITAL_RELEASE_PROVENANCE.md`.
- **Production URL/status:** LIVE Elite `2a4e115` (`index-CiVmQVqq.js` = Capital `b9806bc` + Client `0ffb645`) at `https://zealous-rock-0090c7e1e.7.azurestaticapps.net`; LIVE Hub `b333fb4` at `https://app-atlas-integration-hub.azurewebsites.net` (deploy `5d3826c0`). Do not treat `origin/main` (`b641fdd`) as Azure production. CRM `/leads` Owner-browser **PASS**. Convert is repo-only. stash0 `773e120` is **not applied**. Command-K SYN search P2 (**15–24s**) is not fixed.
