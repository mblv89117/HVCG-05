...
**LIVE (2026-08-19):** Hub `/health` already reports `capitalBackend.mode=sharepoint` on `b333fb4`. Unset/code default remains fail-closed `unavailable`. **ACCG01 ACL Apply was not run** — do not treat health mode as grant proof. Capital Elite `b9806bc` is in SWA `2a4e115`. CRM operator is LIVE; Owner-browser `/leads` PASS. Confirmation-gate P1 CLOSED.
