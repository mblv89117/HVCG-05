...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Microsoft MFA window for Atlas Hub + Elite live cutover | Live certification of Phase 5G + client activation cannot complete without owner Entra auth | **Used 2026-08-20 Azure CLI.** Hub now `940a484` / `9b406df7`. SYN01 activation verified. Capital recorded-only submit certified via governed attestation. **Browser Microsoft login still required** for Elite Premium rendered QA. | Keep `b6a3c9c` zip as immediate rollback | Mixed SHA until Elite rematch | Azure CLI closed; browser MFA open |
