...
class TestSprint3Conversion(unittest.TestCase):
    ...
    def test_dev_lists_not_production(self) -> None:
        # Fit/Diagnostic SharePoint list overlay is deferred (canonical SoR pack already exists).
        missing = []
        for name in ("HVCG_FitAssessments", "HVCG_Diagnostics"):
            path = ROOT / f"src/sharepoint/lists/{name}.json"
            if not path.exists():
                missing.append(name)
                continue
        ...
        if missing:
            self.skipTest(
                "Deferred SharePoint list overlay: " + ", ".join(missing)
            )
