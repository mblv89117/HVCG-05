...
WORKTREES = BA_ROOT.parent
ELITE = WORKTREES / "atlas-usable-operating-layer" / "apps" / "atlas-elite-os" / "src"
...
class IntakeNavDiscoverabilityTests(unittest.TestCase):
    ...
    def test_wrong_worktree_cannot_satisfy_nav_contract(self) -> None:
        """Document the UAT fail root cause: :5180 must serve usable-operating-layer."""
        wrong = WORKTREES / "atlas-local-ai-operations" / "apps" / "atlas-elite-os" / "src"
        self.assertTrue(ELITE.exists())
        if wrong.exists():
            self.assertFalse((wrong / "pages" / "ClientIntakeWorkbench.tsx").exists())
